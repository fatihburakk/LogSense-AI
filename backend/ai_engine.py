...
class AIEngine:
    ...
    def _load_ml_models(self):
        ...
        if not models_dir.exists():
            print("⚠️  ai_models klasörü bulunamadı!")
            return
            ...
            if model_path.exists() and vec_path.exists():
                try:
                    ...
                    with open(vec_path, "rb") as f:
                        self.vectorizers[model_type] = pickle.load(f)
                    loaded += 1
                    print(f"  ✅ {model_type:12s} model + vectorizer yüklendi")
                except Exception as e:
                    print(f"  ❌ {model_type} yüklenirken hata: {e}")
            else:
                print(f"  ⚠️  {model_type} model dosyası bulunamadı, atlanıyor.")

        print(f"🤖 AI Engine başlatıldı — {loaded}/{len(MODEL_TYPES)} model aktif")

    # ─── LLM Başlatma ────────────────────────
    def _init_llm(self):
        """OpenAI istemcisini kurar."""
        if not OPENAI_API_KEY:
            print("⚠️  OPENAI_API_KEY bulunamadı. LLM analizi devre dışı.")
            return

        try:
            self.client = OpenAI(api_key=OPENAI_API_KEY)
            print("🧠 OpenAI LLM bağlantısı kuruldu.")
        except Exception as e:
            print(f"❌ LLM başlatma hatası: {e}")
    ...
    def process_log(self, log_entry: dict) -> dict:
        ...
        if model_type in self.models and model_type in self.vectorizers:
            try:
                ...
                # DEBUG: Ham tahmin değerini yazdır
                print(f"  🔬 [{model_type}] prediction={raw_prediction}, is_anomaly={is_anomaly}, level={level}")

                # Anomali skoru hesapla
                if hasattr(model, "decision_function"):
                    raw_score = model.decision_function(X)[0]
                    anomaly_score = max(0.0, min(1.0, 0.5 - float(raw_score)))
                    print(f"  📊 [{model_type}] raw_score={raw_score:.4f}, anomaly_score={anomaly_score:.4f}")
                elif hasattr(model, "score_samples"):
                    raw_score = model.score_samples(X)[0]
                    anomaly_score = max(0.0, min(1.0, 0.5 - float(raw_score)))
                    print(f"  📊 [{model_type}] score_samples={raw_score:.4f}, anomaly_score={anomaly_score:.4f}")
                else:
                    anomaly_score = 0.95 if is_anomaly else 0.05
                    print(f"  📊 [{model_type}] fallback anomaly_score={anomaly_score}")

            except Exception as e:
                print(f"⚠️ ML tahmini hatası ({model_type}): {e}")
        ...
        print(f"  🎯 LLM karar: is_anomaly={is_anomaly}, score={anomaly_score:.4f}, level={level}, threshold={ANOMALY_THRESHOLD}, should_call={should_call_llm}, client={'VAR' if self.client else 'YOK'}")

        if should_call_llm:
            print(f"  🧠 LLM çağrılıyor... ({source}/{model_type})")
            llm_comment = self._ask_llm(source, message, level, model_type, anomaly_score)
            if llm_comment:
                print(f"  ✅ LLM Türkçe yanıt alındı ({len(llm_comment)} karakter)")
            else:
                print(f"  ❌ LLM boş döndü veya hata oluştu")
        else:
            print(f"  ⏭️  LLM atlandı: is_anomaly={is_anomaly}, skor={anomaly_score:.4f}, seviye={level}")
